...
function Info() {
		center = map.getCenter().transform("EPSG:3857", "EPSG:4326");//map의 중심 좌표 값
		level = map.getZoom();//map의 ZoomLevel 값
		extent = map.getExtent().transform("EPSG:3857", "EPSG:4326");//map의 영역의 값

	}


function InfoMapScale() {
    var result ='지도의 축척은 : '+map.getScale()+'';
...
function distance() {
    Info()
    alert("1")
    handler.findWay({"sp" : [start_y,start_x ], "ep" : [ end_y,  end_x] , "zoom" : level , "boundary" : extent});
    alert("2")
...
new QWebChannel(qt.webChannelTransport, function (channel) {
    ...
    handler.getText.connect(function(message) {
        ps = message
        handler.setText("Succeed" + ps)
    });

    handler.app_to_web_send_data2.connect(function(message) {

        alert("app_to_web_send_data2" + message)
        distance2()
});
